...
import org.opennms.netmgt.flows.elastic.IndexStrategy;
...
public class FlowQueryIT {
    ...
    public void canRetrieveTopNApps() throws ExecutionException, InterruptedException {
        final List<TrafficSummary<Application>> appTrafficSummary = flowRepository.getTopNApplications(10, 0, 100).get();
        assertThat(appTrafficSummary, hasSize(2));
        final TrafficSummary<Application> HTTPS = appTrafficSummary.get(0);
        assertThat(HTTPS.getKey().getName(), equalTo("https"));
        assertThat(HTTPS.getBytesIn(), equalTo(210L));
        assertThat(HTTPS.getBytesOut(), equalTo(2100L));

        final TrafficSummary<Application> HTTP = appTrafficSummary.get(1);
        assertThat(HTTP.getKey().getName(), equalTo("http"));
    ...
    public void canRetrieveTopNAppsSeries() throws ExecutionException, InterruptedException {
        // Top 10
        Table<Directional<Application>, Long, Double> appTraffic = flowRepository.getTopNApplicationsSeries(10, 0, 100, 10).get();
        ...
        assertThat(appTraffic.rowKeySet(), containsInAnyOrder(new Directional<>(new Application("https"), true),
                new Directional<>(new Application("https"), false)));
    }
